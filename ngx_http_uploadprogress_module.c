...
static char *ngx_http_report_uploads(ngx_conf_t *cf, ngx_command_t *cmd,
                                     void *conf);
static ngx_int_t ngx_http_uploadprogress_init_zone(ngx_shm_zone_t *shm_zone,
                                                   void *data);
...
static char *ngx_http_uploadprogress_merge_loc_conf(ngx_conf_t *cf,
                                                    void *parent, void *child);
static ngx_int_t
ngx_http_uploadprogress_init_variables_and_templates(ngx_conf_t *cf);

static ngx_int_t ngx_http_uploadprogress_received_variable(
    ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_http_uploadprogress_offset_variable(
    ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_http_uploadprogress_status_variable(
    ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_http_uploadprogress_callback_variable(
    ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data);
static char *ngx_http_upload_progress_set_template(
    ngx_conf_t *cf, ngx_http_uploadprogress_template_t *t, ngx_str_t *source);
static char *ngx_http_track_uploads(ngx_conf_t *cf, ngx_command_t *cmd,
                                    void *conf);
static char *ngx_http_report_uploads(ngx_conf_t *cf, ngx_command_t *cmd,
                                     void *conf);
static char *ngx_http_upload_progress(ngx_conf_t *cf, ngx_command_t *cmd,
                                      void *conf);
static char *ngx_http_upload_progress_template(ngx_conf_t *cf,
                                               ngx_command_t *cmd, void *conf);
static char *ngx_http_upload_progress_java_output(ngx_conf_t *cf,
                                                  ngx_command_t *cmd,
                                                  void *conf);
static char *ngx_http_upload_progress_json_output(ngx_conf_t *cf,
                                                  ngx_command_t *cmd,
                                                  void *conf);
static char *ngx_http_upload_progress_jsonp_output(ngx_conf_t *cf,
                                                   ngx_command_t *cmd,
                                                   void *conf);
...
static ngx_command_t ngx_http_uploadprogress_commands[] = {
    ...
    {ngx_string("upload_progress"), NGX_HTTP_MAIN_CONF | NGX_CONF_TAKE2,
     ngx_http_upload_progress, 0, 0, NULL},

    {ngx_string("track_uploads"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF |
         NGX_CONF_TAKE2,
     ngx_http_track_uploads, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL},

    {ngx_string("report_uploads"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF |
         NGX_CONF_TAKE1,
     ngx_http_report_uploads, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL},

    {ngx_string("upload_progress_content_type"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF |
         NGX_CONF_TAKE1,
     ngx_conf_set_str_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_uploadprogress_conf_t, content_type), NULL},

    {ngx_string("upload_progress_template"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF |
         NGX_CONF_TAKE2,
     ngx_http_upload_progress_template, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_uploadprogress_conf_t, templates), NULL},

    {ngx_string("upload_progress_java_output"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF |
         NGX_CONF_NOARGS,
     ngx_http_upload_progress_java_output, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL},

    {ngx_string("upload_progress_json_output"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF |
         NGX_CONF_NOARGS,
     ngx_http_upload_progress_json_output, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL},

    {ngx_string("upload_progress_jsonp_output"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF |
         NGX_CONF_NOARGS,
     ngx_http_upload_progress_jsonp_output, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL},

    {ngx_string("upload_progress_header"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF |
         NGX_CONF_TAKE1,
     ngx_conf_set_str_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_uploadprogress_conf_t, header), NULL},

    {ngx_string("upload_progress_jsonp_parameter"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF |
         NGX_CONF_TAKE1,
     ngx_conf_set_str_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_uploadprogress_conf_t, jsonp_parameter), NULL},

    ngx_null_command};

static ngx_http_variable_t ngx_http_uploadprogress_variables[] = {

    {ngx_string("uploadprogress_received"), NULL,
     ngx_http_uploadprogress_received_variable,
    ...
    {ngx_string("uploadprogress_remaining"), NULL,
     ngx_http_uploadprogress_offset_variable,
     (uintptr_t)offsetof(ngx_http_uploadprogress_node_t, rest),
     NGX_HTTP_VAR_CHANGEABLE | NGX_HTTP_VAR_NOCACHEABLE | NGX_HTTP_VAR_NOHASH,
     0},

    {ngx_string("uploadprogress_length"), NULL,
     ngx_http_uploadprogress_offset_variable,
     (uintptr_t)offsetof(ngx_http_uploadprogress_node_t, length),
     NGX_HTTP_VAR_CHANGEABLE | NGX_HTTP_VAR_NOCACHEABLE | NGX_HTTP_VAR_NOHASH,
     0},

    {ngx_string("uploadprogress_status"), NULL,
     ngx_http_uploadprogress_status_variable,
     (uintptr_t)offsetof(ngx_http_uploadprogress_node_t, err_status),
     NGX_HTTP_VAR_CHANGEABLE | NGX_HTTP_VAR_NOCACHEABLE | NGX_HTTP_VAR_NOHASH,
     0},

    {ngx_string("uploadprogress_callback"), NULL,
     ngx_http_uploadprogress_callback_variable, (uintptr_t)NULL,
     NGX_HTTP_VAR_CHANGEABLE | NGX_HTTP_VAR_NOCACHEABLE | NGX_HTTP_VAR_NOHASH,
     0},
...
static ngx_str_t ngx_http_uploadprogress_java_defaults[] = {
    ...
    ngx_string("new Object({ 'state' : 'error', 'status' : "
...
static ngx_str_t ngx_http_uploadprogress_json_defaults[] = {
    ...
    ngx_string(
        "{ \"state\" : \"error\", \"status\" : $uploadprogress_status }\r\n"),
    ngx_string("{ \"state\" : \"done\" }\r\n"),
    ngx_string(
        "{ \"state\" : \"uploading\", \"received\" : $uploadprogress_received, "
        "\"size\" : $uploadprogress_length }\r\n")};

static ngx_str_t ngx_http_uploadprogress_jsonp_defaults[] = {
    ngx_string("$uploadprogress_callback({ \"state\" : \"starting\" });\r\n"),
    ngx_string("$uploadprogress_callback({ \"state\" : \"error\", \"status\" : "
               "$uploadprogress_status });\r\n"),
    ngx_string("$uploadprogress_callback({ \"state\" : \"done\" });\r\n"),
    ngx_string(
        "$uploadprogress_callback({ \"state\" : \"uploading\", \"received\" : "
        "$uploadprogress_received, \"size\" : $uploadprogress_length });\r\n")};

static ngx_array_t ngx_http_uploadprogress_global_templates;

static ngx_str_t *get_tracking_id(ngx_http_request_t *r) {
  u_char *p, *start_p;
  ngx_uint_t i;
  ...
  ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                 "upload-progress: get_tracking_id");
  ...
  /* it is possible the request args have not been yet created (or already
   * released) */
...
find_node(ngx_str_t *id, ngx_http_uploadprogress_ctx_t *ctx, ngx_log_t *log) {
  ...
  ngx_log_debug1(NGX_LOG_DEBUG_HTTP, log, 0, "upload-progress: find_node %V",
                 id);
  ...
  while (node != sentinel) {
    ...
    /* found a key with unmatching hash (and value), let's keep comparing hashes
     * then */
    if (rc < 0) {
      node = node->left;
      continue;
...
ngx_http_uploadprogress_content_handler(ngx_http_request_t *r) {
  ...
  ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                 "upload-progress: ngx_http_uploadprogress_content_handler");
  upcf = ngx_http_get_module_loc_conf(r, ngx_http_uploadprogress_module);
  ...
  /* request is OK, hijack the read_event_handler if the request has to be
   * tracked*/
...
static ngx_str_t *ngx_http_uploadprogress_strdup(ngx_str_t *src,
                                                 ngx_log_t *log) {
  ngx_str_t *dst;
...
static void ngx_http_uploadprogress_event_handler(ngx_http_request_t *r) {
  ...
  ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                 "upload-progress: ngx_http_uploadprogress_event_handler");
  ...
  /* and thats the same issue with any other material that was allocated in the
   * request pool */
  /* that's why we duplicate id afterward */

  /* it's also possible that the id was null if we got a spurious (like abort)
   * read */
  /* event. In this case we still have called the original read event handler */
  /* but we have to bail out, because we won't ever be able to find our upload
   * node */

  if (shm_zone == NULL) {
    ngx_http_uploadprogress_strdupfree(id);
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, ngx_cycle->log, 0,
                   "upload-progress: read_event_handler no shm_zone for id: %V",
                   id);
    return;
  }
...
static ngx_int_t ngx_http_reportuploads_handler(ngx_http_request_t *r) {
  ...
  if (up != NULL) {
    ngx_log_debug5(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "reportuploads found node: %V (rest: %uO, length: %uO, "
                   "done: %ui, err_status: %ui)",
                   id, up->rest, up->length, up->done, up->err_status);
    rest = up->rest;
    length = up->length;
    done = up->done;
...
*/
static ngx_int_t ngx_http_uploadprogress_handler(ngx_http_request_t *r) {
  size_t n;
  ngx_str_t *id;
  uint32_t hash;
  ngx_slab_pool_t *shpool;
  ...
  cln =
      ngx_pool_cleanup_add(r->pool, sizeof(ngx_http_uploadprogress_cleanup_t));
  if (cln == NULL) {
    ngx_shmtx_unlock(&shpool->mutex);
    ngx_free(id);
    return NGX_HTTP_INTERNAL_SERVER_ERROR;
...
static void ngx_clean_old_connections(ngx_event_t *ev) {
  ...
  ngx_log_debug3(
      NGX_LOG_DEBUG_HTTP, shm_zone->shm.log, 0,
      "uploadprogress clean old connections: quit: %ui term: %ui count: %ui",
      ngx_quit, ngx_terminate, count);
  ...
  if (count > 0 || !(ngx_quit || ngx_terminate || ngx_exiting)) {
    ...
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, shm_zone->shm.log, 0,
                   "uploadprogress clean old connections quitting , no more "
                   "active connections: not restarting timer");
  }
  ngx_shmtx_unlock(&shpool->mutex);
}
...
static ngx_int_t ngx_http_uploadprogress_errortracker(ngx_http_request_t *r) {
  size_t n;
  ngx_str_t *id;
  ngx_slab_pool_t *shpool;
  ngx_rbtree_node_t *node;
  ...
  if (r->err_status >= NGX_HTTP_SPECIAL_RESPONSE) {
    ...
    cln = ngx_pool_cleanup_add(r->pool,
                               sizeof(ngx_http_uploadprogress_cleanup_t));
    if (cln == NULL) {
      ngx_shmtx_unlock(&shpool->mutex);
      ngx_free(id);
      goto finish;
...
static void *ngx_http_uploadprogress_create_loc_conf(ngx_conf_t *cf) {
  ...
  if (ngx_array_init(&conf->templates, cf->pool, 4,
...
static char *ngx_http_uploadprogress_merge_loc_conf(ngx_conf_t *cf,
  ...
  ngx_conf_merge_str_value(conf->jsonp_parameter, prev->jsonp_parameter,
                           "callback");
...
ngx_http_uploadprogress_init_variables_and_templates(ngx_conf_t *cf) {
  ...
  while (m->name.data != NULL) {
    ngx_http_uploadprogress_template_t *elt =
        ngx_array_push(&ngx_http_uploadprogress_global_templates);
    ngx_http_script_variables_count(ngx_http_uploadprogress_jsonp_defaults + i);

    if (ngx_http_upload_progress_set_template(
...
static char *ngx_http_upload_progress(ngx_conf_t *cf, ngx_command_t *cmd,
  ...
  ngx_log_debug1(NGX_LOG_DEBUG_HTTP, cf->log, 0, "ngx_upload_progress name: %V",
                 &value[1]);
  ...
  shm_zone =
      ngx_shared_memory_add(cf, &value[1], n, &ngx_http_uploadprogress_module);
  if (shm_zone == NULL) {
    return NGX_CONF_ERROR;
  }
...
static char *ngx_http_track_uploads(ngx_conf_t *cf, ngx_command_t *cmd,
  ...
  if (lzcf->handler == NULL) {
    return "track_upload should be the last directive in the location, after "
           "either proxy_pass or fastcgi_pass";
  }
...
static char *ngx_http_upload_progress_template(ngx_conf_t *cf,
  ...
  while (m->name.data != NULL) {
    if ((value[1].len == m->name.len &&
         !ngx_strncmp(value[1].data, m->name.data, m->name.len)) ||
  ...
  if (m->name.data == NULL) {
    ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "unknown state \"%V\"", &value[1]);
    return NGX_CONF_ERROR;
  }
